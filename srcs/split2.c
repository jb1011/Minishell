...
static unsigned int	ft_get_nb_strs(char const *s, char c, char d, char f)
{
	unsigned int	i;
...
static void	ft_get_next_str(char **next_str, unsigned int *next_str_len, char c)
{
	...
	*next_str_len = 0;
	i = 0;
	while (**next_str && **next_str == c && **next_str == d && **next_str == f)
		(*next_str)++;
	while ((*next_str)[i])
	{
		if ((*next_str)[i] == c || (*next_str)[i] == d || (*next_str)[i] == f)
			return ;
		(*next_str_len)++;
		i++;
	}
